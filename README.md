...
- **Modular:** front-end estático + un gateway. Cambiar de proveedor o modelo = editar
  un objeto en `worker/worker.js`. El front-end se adapta solo.
...
Para bloquear orígenes ajenos, edita `ALLOWED_ORIGIN` en `worker/wrangler.toml`
(ej. `"https://c48484518.github.io"`) y vuelve a `npx wrangler deploy`.

### 3. Conecta el front-end

En `assets/config.js`, pon tu URL del Worker en `GATEWAY_URL`
(y `APP_TOKEN` si configuraste ese secret).

### 4. Publica el front-end

Sube `index.html` y la carpeta `assets/` a tu repo y activa GitHub Pages. Listo.

## Añadir o cambiar proveedores / modelos

Edita el objeto `PROVIDERS` en `worker/worker.js`. Cada modelo:
